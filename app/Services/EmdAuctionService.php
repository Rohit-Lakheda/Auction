...
class EmdAuctionService
{
    ...
    public function releaseUserEmdIfEligible(int $auctionId, int $userId): array
    {
        return DB::transaction(function () use ($auctionId, $userId): array {
            ...
            $ended = in_array((string) $auction->status, ['closed', 'completed', 'failed'], true)
    ...
    public function markTopBidderDefaultAndPromote(int $auctionId): void
    {
        DB::transaction(function () use ($auctionId): void {
            ...
            $top = $this->getTopBidders($auctionId);
            if (empty($top)) {
                DB::table('auctions')->where('id', $auctionId)->update(['status' => 'failed', 'payment_status' => 'failed', 'updated_at' => now()]);
                return;
            }
            ...
            $participant = DB::table('auction_participants')
                ->where('auction_id', $auctionId)
                ->where('user_id', (int) $defaulter['user_id'])
                ->lockForUpdate()
                ->first();
            if (! $participant) {
                throw new RuntimeException('Defaulter participant record not found.');
            }

            $locked = (float) $participant->locked_emd_amount;
            $penaltyPercent = $this->settingsService->getFloat('emd_penalty_percentage', (float) config('emd.penalty_percentage', 25));
            $penalty = round(($locked * max(0, $penaltyPercent)) / 100, 2);
            $refund = max(0, $locked - $penalty);

            if ($refund > 0) {
                $this->walletService->refundEmd((int) $defaulter['user_id'], $auctionId, $refund, 'Partial EMD refund after default');
            }
            $this->walletService->applyPenalty((int) $defaulter['user_id'], $auctionId, $penalty);

            DB::table('auction_participants')
                ->where('id', $participant->id)
                ->update(['status' => 'defaulted', 'emd_locked' => 0, 'updated_at' => now()]);

            DB::table('users')
                ->where('id', (int) $defaulter['user_id'])
                ->increment('default_count', 1);

            $defaults = (int) (DB::table('users')->where('id', (int) $defaulter['user_id'])->value('default_count') ?? 0);
            $maxDefaultBeforeBlock = $this->settingsService->getInt('emd_max_default_before_block', (int) config('emd.max_default_before_block', 3));
            if ($defaults >= $maxDefaultBeforeBlock) {
                DB::table('users')->where('id', (int) $defaulter['user_id'])->update(['is_blocked' => 1, 'updated_at' => now()]);
            } else {
                DB::table('users')->where('id', (int) $defaulter['user_id'])->update([
                    'emd_multiplier' => DB::raw('LEAST(5, GREATEST(1, emd_multiplier + 0.5))'),
                    'updated_at' => now(),
                ]);
            }

            $remaining = array_values(array_filter($top, static fn (array $row): bool => (int) $row['user_id'] !== (int) $defaulter['user_id']));
            if (empty($remaining)) {
                DB::table('auctions')->where('id', $auctionId)->update([
                    'status' => 'failed',

...
class PaymentController extends Controller
{
    ...
    public function auctionSuccess(Request $request, PayuService $payu)
    {
        ...
        preg_match('/AUCTION_(\d+)/', (string) ($data['udf1'] ?? ''), $m);
        $auctionId = (int) ($m[1] ?? 0);
        $userId = (int) ($data['udf2'] ?? 0);

        DB::transaction(function () use ($txnid, $data, $auctionId, $userId): void {
            ...
            DB::table('auctions')
                ->where('id', $auctionId)
                ->where('winner_user_id', $userId)
                ->update(['status' => 'completed', 'payment_status' => 'paid', 'payment_date' => now()]);
    ...
    public function registrationFailure(Request $request)
    {
        ...
        return redirect()->route('register')->withErrors(['payment' => 'Payment failed. Please try again.']);
    }

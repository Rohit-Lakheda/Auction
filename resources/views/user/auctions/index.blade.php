...
@section('content')
    ...
    <div class="card">
        ...
        <form method="GET" class="filters-panel">
            ...
            <div>
                <label class="filter-label">EMD Filter</label>
                <select class="filter-control" name="emd">
                    <option value="all" {{ $emdFilter==='all' ? 'selected' : '' }}>All</option>
                    <option value="low" {{ $emdFilter==='low' ? 'selected' : '' }}>Low (<= 5,000)</option>
                    <option value="mid" {{ $emdFilter==='mid' ? 'selected' : '' }}>Mid (5,001 - 25,000)</option>
                    <option value="high" {{ $emdFilter==='high' ? 'selected' : '' }}>High (> 25,000)</option>
                </select>
            </div>
            <div>
                ...
                <div class="watchlist-grid">
                    @foreach($watchlistAuctions as $w)
                        <div class="watch-item">
                            ...
                            <div class="watch-meta">EMD: ₹{{ number_format((float) ($w->emd_amount ?? 0), 2) }}</div>
        ...
        <div class="grid">
            @foreach ($auctions as $auction)
                @php
                    ...
                    $joined = (int)($auction->joined_count ?? 0) > 0;
                ...
                <div class="auction-card">
                    ...
                    <div style="border-top: 1px solid #e2e8f0; border-bottom: 1px solid #e2e8f0; padding: 15px 0; margin: 15px 0;">
                        ...
                        <div style="display: flex; justify-content: space-between; margin-top: 10px;">
                            <span style="color: #718096;">EMD:</span>
                            <strong>₹{{ number_format((float)($auction->emd_amount ?? 0), 2) }}</strong>
                        </div>
                    </div>
                    ...
                    @if($joined)
                        <div class="alert alert-success" style="padding:8px 10px;">Your EMD is locked. Ready to bid.</div>
                    @endif

                    <div style="display:flex;gap:8px;">
                        <a href="{{ route('user.auctions.show', $auction->id) }}" class="btn" style="flex:1;text-align:center;">View Details</a>
                        @if(!$joined)
                            <form method="POST" action="{{ route('user.auctions.join', $auction->id) }}" style="flex:1;">
                                @csrf
                                <button type="submit" class="btn btn-success" style="width:100%;">Join</button>
                            </form>
                        @endif
                    </div>

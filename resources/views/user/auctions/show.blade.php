...
@section('content')
    ...
    <div class="card">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin: 20px 0;">
            ...
            {{-- [EMD DISABLED] Required EMD box removed --}}
        </div>

        <div id="auctionStatusBox" class="alert alert-info" style="margin-top: 10px;">Loading your rank/status...</div>
        @if(!$isParticipant)
            <div class="alert alert-info">
                Participation fee required: <strong>₹{{ number_format($requiredEmd, 2) }}</strong>.
                Pay now to unlock bidding for this auction.
            </div>
            <form method="POST" action="{{ route('user.auctions.join', $auction->id) }}" style="margin-top:10px;">
                @csrf
                <button type="submit" class="btn btn-success">Pay Participation Fee & Join</button>
            </form>
        @endif
    </div>
    ...
    <div class="card" style="position:sticky;top:20px;z-index:2;">
        ...
        @else
            <div class="alert alert-info">Complete participation payment to enable bidding for this auction.</div>
        @endif
    </div>

...
<div class="card">
    ...
    <div class="tabs">
        ...
        <button type="button" class="tab {{ $activeTab==='emd'?'active':'' }}" onclick="switchTab('emd',this)">EMD Settings</button>
    ...
    <div id="tab-emd" class="tab-content {{ $activeTab==='emd'?'active':'' }}">
        <form method="POST">@csrf<input type="hidden" name="form_type" value="emd_settings">
            <div class="form-group"><label>Default EMD Amount (₹) *</label><input type="number" name="emd_default_amount" step="0.01" min="0" required value="{{ number_format((float) $emdSettings['emd_default_amount'],2,'.','') }}"></div>
            <div class="form-group"><label>Penalty Percentage (%) *</label><input type="number" name="emd_penalty_percentage" step="0.01" min="0" max="100" required value="{{ number_format((float) $emdSettings['emd_penalty_percentage'],2,'.','') }}"></div>
            <div class="form-group"><label>Payment Window (Hours) *</label><input type="number" name="emd_payment_window_hours" min="1" max="168" required value="{{ (int) $emdSettings['emd_payment_window_hours'] }}"></div>
            <div class="form-group"><label>Max Defaults Before Block *</label><input type="number" name="emd_max_default_before_block" min="1" max="20" required value="{{ (int) $emdSettings['emd_max_default_before_block'] }}"></div>
            <div class="form-group"><label>Default EMD Multiplier *</label><input type="number" name="emd_default_multiplier" step="0.01" min="1" max="10" required value="{{ number_format((float) $emdSettings['emd_default_multiplier'],2,'.','') }}"></div>
            <button class="btn" type="submit">Update EMD Settings</button>
        </form>
    </div>

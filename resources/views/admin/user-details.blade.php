...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:12px;">
    ...
    <div class="card" style="padding:18px;"><div style="color:#6c757d;">Joined Auctions</div><div style="font-size:24px;color:#1a237e;">{{ $stats['joined_auctions'] }}</div></div>
    <div class="card" style="padding:18px;"><div style="color:#6c757d;">Wallet Balance</div><div style="font-size:24px;color:#1a237e;">₹{{ number_format((float)$stats['wallet_balance'],2) }}</div></div>
</div>
...
<div class="card">
    <form method="GET" style="display:flex;justify-content:space-between;align-items:end;gap:10px;flex-wrap:wrap;margin-bottom:10px;">
        @foreach($query as $k => $v)
            @if(! in_array($k, ['participation_per_page', 'participation_page'], true))
                <input type="hidden" name="{{ $k }}" value="{{ is_array($v) ? json_encode($v) : $v }}">
            @endif
        @endforeach
        <h3 style="color:#1a237e;margin:0;">Auction Participation (EMD)</h3>
        <div class="form-group" style="margin:0;">
            <label>Per Page</label>
            <select name="participation_per_page" onchange="this.form.submit()">@foreach(['10','20','50','100','all'] as $size)<option value="{{ $size }}" {{ ($perPage['participation'] ?? '10') === $size ? 'selected' : '' }}>{{ strtoupper($size) }}</option>@endforeach</select>
        </div>
    </form>
    @if($participations->isEmpty()) <p>No participation records found.</p> @else
    <div style="overflow:auto;"><table>
        <thead><tr><th>Auction</th><th>Auction Status</th><th>Participant Status</th><th>EMD Locked</th><th>Locked Amount</th><th>Joined At</th></tr></thead>
        <tbody>@foreach($participations as $p)<tr>
            <td>{{ $p->title }}</td><td>{{ strtoupper($p->auction_status) }}</td><td>{{ strtoupper($p->status) }}</td>
            <td>{{ (int)$p->emd_locked === 1 ? 'Yes' : 'No' }}</td><td>₹{{ number_format((float)$p->locked_emd_amount,2) }}</td>
            <td>{{ $p->joined_at ? \Carbon\Carbon::parse($p->joined_at)->format('d-M-Y H:i') : '-' }}</td>
        </tr>@endforeach</tbody>
    </table></div>@include('partials.grid-pagination', ['rows' => $participations])@endif
</div>

<div class="card">
    <form method="GET" style="display:flex;justify-content:space-between;align-items:end;gap:10px;flex-wrap:wrap;margin-bottom:10px;">
        @foreach($query as $k => $v)
            @if(! in_array($k, ['wallet_txn_per_page', 'wallet_txn_page'], true))
                <input type="hidden" name="{{ $k }}" value="{{ is_array($v) ? json_encode($v) : $v }}">
            @endif
        @endforeach
        <h3 style="color:#1a237e;margin:0;">Wallet Transactions</h3>
        <div class="form-group" style="margin:0;">
            <label>Per Page</label>
            <select name="wallet_txn_per_page" onchange="this.form.submit()">@foreach(['10','20','50','100','all'] as $size)<option value="{{ $size }}" {{ ($perPage['wallet_txn'] ?? '10') === $size ? 'selected' : '' }}>{{ strtoupper($size) }}</option>@endforeach</select>
        </div>
    </form>
    @if($walletTransactions->isEmpty()) <p>No wallet transactions found.</p> @else
    <div style="overflow:auto;"><table>
        <thead><tr><th>Type</th><th>Amount</th><th>Reference</th><th>Status</th><th>Remarks</th><th>Date</th></tr></thead>
        <tbody>@foreach($walletTransactions as $t)<tr>
            <td>{{ strtoupper((string)$t->type) }}</td><td>₹{{ number_format((float)$t->amount,2) }}</td>
            <td>{{ ($t->reference_type ?? '-') . ' #' . ($t->reference_id ?? '-') }}</td><td>{{ strtoupper((string)$t->status) }}</td>
            <td>{{ $t->remarks ?? '-' }}</td><td>{{ \Carbon\Carbon::parse($t->created_at)->format('d-M-Y H:i') }}</td>
        </tr>@endforeach</tbody>
    </table></div>@include('partials.grid-pagination', ['rows' => $walletTransactions])@endif
</div>

<div class="card">
    <form method="GET" style="display:flex;justify-content:space-between;align-items:end;gap:10px;flex-wrap:wrap;margin-bottom:10px;">
        @foreach($query as $k => $v)
            @if(! in_array($k, ['wallet_topup_per_page', 'wallet_topup_page'], true))
                <input type="hidden" name="{{ $k }}" value="{{ is_array($v) ? json_encode($v) : $v }}">
            @endif
        @endforeach
        <h3 style="color:#1a237e;margin:0;">Wallet Top-ups</h3>
        <div class="form-group" style="margin:0;">
            <label>Per Page</label>
            <select name="wallet_topup_per_page" onchange="this.form.submit()">@foreach(['10','20','50','100','all'] as $size)<option value="{{ $size }}" {{ ($perPage['wallet_topup'] ?? '10') === $size ? 'selected' : '' }}>{{ strtoupper($size) }}</option>@endforeach</select>
        </div>
    </form>
    @if($walletTopups->isEmpty()) <p>No wallet top-ups found.</p> @else
    <div style="overflow:auto;"><table>
        <thead><tr><th>Transaction ID</th><th>Amount</th><th>Status</th><th>Gateway Txn</th><th>Date</th></tr></thead>
        <tbody>@foreach($walletTopups as $t)<tr>
            <td>{{ $t->transaction_id }}</td><td>₹{{ number_format((float)$t->amount,2) }}</td><td>{{ strtoupper((string)$t->status) }}</td>
            <td>{{ $t->gateway_transaction_id ?? '-' }}</td><td>{{ \Carbon\Carbon::parse($t->created_at)->format('d-M-Y H:i') }}</td>
        </tr>@endforeach</tbody>
    </table></div>@include('partials.grid-pagination', ['rows' => $walletTopups])@endif
</div>
